...
_label_seen = defaultdict(lambda: {
    "any": set(),
    SKOS.prefLabel: set(),
    RDFS.label: set(),
})
...
def _norm_label_text(t: str) -> str:
    return re.sub(r"\s+", " ", (t or "")).strip().casefold() # normalize whitespace and case
...
def add_label(g: Graph, node, prop, text: str) -> bool:
    ...
    cross_allowed = (
        (prop == RDFS.label and t in seen[SKOS.prefLabel]) or
...
pmid_rx = re.compile(r"^\s*PMID\s*:\s*(\d+)\s*$", re.IGNORECASE) # matches "PMID: 12345" format
...
def to_pubmed_urls(text, *, src_file=None, row=None, col=None):
    ...
    for bad in ["\n", "\r", "\t"]:
        if bad in text:
            loc = []
            if src_file: loc.append(f"file={src_file}")
            if row is not None: loc.append(f"row={row}")
            if col: loc.append(f"col={col}")
    ...
    for slot in (s.strip() for s in text.split("|")):
        ...
        for tok in (t.strip() for t in slot.split(";")):
            ...
            elif tok.lower().startswith("http://") or tok.lower().startswith("https://"):
                urls.append(tok)
    seen, keep = set(), []
    for u in urls:
        if u not in seen:
...
def make_valid(s):
    return (s or "").replace(" ", "_").replace(":", "_").replace("/", "_").replace("|","_") # make string safe for URI use
...
def canon_uniprot_id(val):
    ...
    if orig and orig.upper() != s:
        alts.add(orig)
    return base, s, sorted(a for a in alts if a) # returns uniprot base, uniprot full, [alt ids]
...
def canon_chebi_id(val):
    ...
    if not s.upper().startswith("CHEBI:"):
        raise ValueError(f"Expected 'CHEBI:' prefix (or URL ending with it), got: {val!r}")
...
def read_code_names_uniprot(path):
    ...
    for r in rows:
        ...
        base = uid.split("-")[0]
        if nm:
            if base not in id2name: id2name[base] = nm
            if uid  not in id2name: id2name[uid]  = nm
        if url:
            if base not in id2url: id2url[base] = url
            if uid  not in id2url: id2url[uid]  = url
...
def read_code_names_umls(path):
    ...
    for r in rows:
        ...
        nm  = (r.get("name") or "").strip()
        url = (r.get("url")  or "").strip()
        if cui and nm  and cui not in id2name: id2name[cui] = nm
        if cui and url and cui not in id2url:  id2url[cui]  = url
...
def read_code_names_chebi(path):
    ...
    for r in rows:
        ...
        if nm  and code_colon and code_colon not in id2name: id2name[code_colon] = nm
        if url and code_colon and code_colon not in id2url:  id2url[code_colon]  = url
...
def add_reified_relation(g, subj, pred, obj, prov_str):
    ...
    if prov in document_map:
        ...
        doc = MAKAAO[f"document_{len(document_map)+1}"] # otherwise we create a new instance of Document, and we construct its URI based on the number of documents already known
    ...
    for rel in g.subjects(RDF.subject, subj):
        if (rel, RDF.predicate, pred) in g and (rel, RDF.object, obj_node) in g: # if the relation already exist, there is nothing to do
            existing_rel = rel
            break

    if existing_rel is None:
        rel = MAKAAO[f"r{relation_counter}"]; relation_counter += 1 # if that relation does not exist yet, we create it, and increment the relation counter
        g.add((rel, RDF.type, MAKAAO.Relation))
        g.add((rel, RDF.type, RDF.Statement))
        g.add((rel, RDF.subject, subj)) # we add some triples about the new reified relation
    ...
    g.add((rel, PROV.wasDerivedFrom, doc)) # we add the provenance information to the reified relation
...
def umls_code_group(path, key="CUI"): # read UMLS code names table into two dicts grouped by key and by CODE
...
def init_graph(): # start an empty knowledge graph and add a few basic things to it
    g = Graph()
    # Prefixes
    g.bind("mak", MAKAAO); g.bind("rdfs", RDFS); g.bind("owl", OWL)
    g.bind("skos", SKOS);  g.bind("prov", PROV); g.bind("hp", HP)
    g.bind("biolink", BIOLINK); g.bind("bao", BAO); g.bind("sio", SIO)
    g.bind("dcterms", DCTERMS); g.bind("dcat", DCAT); g.bind("odrl", ODRL)
    g.bind("schema", SCHEMA); g.bind("void", VOID); g.bind("foaf", FOAF)
...
def load_processed_tables(base_dir):
    ...
    name_fr_path = os.path.join(base_dir, "index_name_source_fr.csv")
    ...
    name_fr   = read_csv_rows(name_fr_path)
    ...
    for i, r in enumerate(cui_src, start=2):
        ...
        pm  = to_pubmed_urls(r.get("umls_pmids"), src_file=cui_path, row=i, col="umls_pmids")
        if idx and cui:
            cui_by_idx[idx].append((cui, pm))

    uni_by_idx = defaultdict(list) # dict: {aab_id: [(uniprot_id, [pmid_url, ...]), ...]}
    for i, r in enumerate(uni_src, start=2):
        idx = (r.get("index") or "").strip()
        if idx not in KEEP: continue
        uid = (r.get("uniprot_target_id") or "").strip()
        pm  = to_pubmed_urls(r.get("uniprot_pmids"), src_file=uni_path, row=i, col="uniprot_pmids")
        if idx and uid:
    ...
    for i, r in enumerate(che_src, start=2):
        ...
        pm  = to_pubmed_urls(r.get("chebi_pmids"), src_file=che_path, row=i, col="chebi_pmids")
        if idx and cid:
            che_by_idx[idx].append((cid, pm))

    dis_by_idx = defaultdict(list) # dict: {aab_id: [(disease_id, [pmid_url, ...]), ...]}
    for i, r in enumerate(dis_src, start=2):
        idx = (r.get("index") or "").strip()
        if idx not in KEEP: continue
        did = (r.get("related_diseases_id") or "").strip()
        pm  = to_pubmed_urls(r.get("diseases_pmids"), src_file=dis_path, row=i, col="diseases_pmids")
        if idx and did:
...
def build_core(g, data, umls_hpo_rows, umls_names, up_names, up_urls, umls_cn_names, chebi_cn_names, chebi_cn_urls):
    # Target class specializations
    uniprot_protein = URIRef("http://purl.uniprot.org/core/Protein") # define the root classes for uniprot proteins and chebi molecules
    ...
    umls_hpo_by_code = grouped(umls_hpo_rows, "CODE") if umls_hpo_rows else {} # create index of umls_hpo by code
    primary_labels, aab_class_uri = {}, {}
    pos_uris_by_idx = {}

    # Build classes/instances
    for idx in sorted(data["indices"], key=lambda x: int(x)):
        cls = MAKAAO.Autoantibody if idx == "18" else MAKAAO[f"aab_{idx}"] # 18 is autoimmune antibody: the index of the root class for all autoantibodies
        if idx != "18":
        ...
        for syn, src in data["syn_en"].get(idx, []):
            if add_label(g, cls, RDFS.label, syn) and src: # add english as syns as rdfs:labels, and add corresponding reified relation with provenance (if there is provenance)
        ...
        if idx != "18":  # root AAb has no parent
            ...
            if valid:
                for p in valid:
                    parent_uri = MAKAAO.Autoantibody if p == "18" else MAKAAO[f"aab_{p}"] # we reconstruct the URI of the parent class from its ID
                    g.add((cls, RDFS.subClassOf, parent_uri)) # we add the subsumption triple
            else:
                g.add((cls, RDFS.subClassOf, MAKAAO.Autoantibody)) # if no valid parent, we link to root AAb


        # instance
        inst = MAKAAO[f"aab_{idx}_instance"]
        g.add((inst, RDF.type, cls)) # we create an instance of the current AAb class with its preferred label
        add_pref(g, inst, primary_labels[idx])

        # positivity (HPO-backed if present)
        pos_list = []
        for hp_code in data["hpo_list"].get(idx, []): # we try to get the associated HPO positivity(ies) for that AAb, if it exits
            pos_uri = hp_to_obo_uri(hp_code)
            if not pos_uri:
                continue
            code_norm = hp_code.replace("_", ":").upper()
            pt_label = None
            for row in umls_hpo_by_code.get(code_norm, []): #we try to get the main enlgish name for the HPO code from our UMLS-HPO table
                s = (row.get("STR") or "").strip()
                tty = (row.get("TTY") or "").strip().upper()
                if not s: continue
                if tty == "PT" and pt_label is None:
                    pt_label = s; add_pref(g, pos_uri, s)
                add_label(g, pos_uri, RDFS.label, s) # if we find it, we add it as a skos:prefLabel and rdfs:label
            if pt_label is None:
                add_pref(g, pos_uri, code_norm) # if we don't find a preferred name, we just add the code as prefLabel
        ...
        pos_uris_by_idx[idx] = [u for (u, _) in pos_list] # positivity URIS associated to each aab_id

        # positivity instances and biolink
        n = len(pos_list)
        for j, (pos_uri, pos_label) in enumerate(pos_list, start=1): # for each positivity associated to current AAb, we create an instance and link it with biolink relations
    ...
    for idx, parents in data["parents"].items():
        child_list = pos_uris_by_idx.get(idx, [MAKAAO[f"positivity_{idx}"]]) # we get the positivity URIS for the current AAb
        for p in parents:
            if p not in data["indices"]:
                continue
            parent_list = pos_uris_by_idx.get(p, [MAKAAO[f"positivity_{p}"]]) # for eahc parent of the current aab, we get the corresponding positivity URI
            for c_uri in child_list:
                for p_uri in parent_list:
                    g.add((c_uri, RDFS.subClassOf, p_uri)) # we make the subsumption link between the child and parent positivity URIs

    # UMLS targets
    umls_local_names = umls_cn_names or {} # umls_names from code_names table
    for idx, items in data["cui"].items():
        inst_aab = MAKAAO[f"aab_{idx}_instance"] # for each aab_id, we get the name of the corresponding instance
        for cui, pmids in items:
            cui_key = cui.upper().replace("CUI:", "") # for each CUI associated to this aab_id
            cui_uri = URIRef(UMLS + cui_key + "_instance") # we create the URI of the CUI instanceS
            g.add((cui_uri, RDF.type, MAKAAO.Target)) # we create a instance of the Target class with the relevant CUI as URI
            all_umls_labels = (umls_names.get(cui_key) or []) # we read from umls_names table all the names associated to that CUI
            preferred = umls_local_names.get(cui_key) or (all_umls_labels[0] if all_umls_labels else None) # we try tp get preferred name from code_names table, or else from umls_name table
            if preferred: add_pref(g, cui_uri, preferred)
            else:         add_pref(g, cui_uri, cui_key) # if we find a pref name, we add it as pref label, else we take the CUI as label
            for n in all_umls_labels:
                add_label(g, cui_uri, RDFS.label, n) # we add all the other names as rdfs:labels
            add_label(g, cui_uri, RDFS.label, cui_key)
            g.add((inst_aab, BAO.BAO_0000211, cui_uri)) # we add relations between target instance and AAb instance
            g.add((cui_uri, BAO.BAO_0000598, inst_aab))
            for p in (pmids or [""]):
                add_reified_relation(g, inst_aab, BAO.BAO_0000211, cui_uri, p) # for each source we have, we add a reified relation to carry provenance
                add_reified_relation(g, cui_uri, BAO.BAO_0000598, inst_aab, p)

    # UniProt targets
    up_names_added, up_total = 0, 0
    for idx, items in data["uniprot"].items(): # for each aab_id, we get the associated uniprot ids
        inst_aab = MAKAAO[f"aab_{idx}_instance"]
        for upid_in, pmids in items:
            up_total += 1
            base, norm, _ = canon_uniprot_id(upid_in) # we canonicalize the uniprot id read from data
            if not base: continue
            prot_ind = MAKAAO[f"{make_valid('UP_'+base+'_instance')}"] # we create the URI of the uniprot protein instance
            g.add((prot_ind, RDF.type, URIRef("http://purl.uniprot.org/core/Protein"))) # we declare it as an instance of uniprot protein class
            up_name = up_names.get(base) or up_names.get(norm) # we try to get the name of the uniprot protein from code_names table
            if up_name:
                add_pref(g, prot_ind, up_name)
                add_label(g, prot_ind, RDFS.label, base) # we add prefLabel and rdfs:label if we find a name
                up_names_added += 1
            else:
                add_pref(g, prot_ind, base)
            up_url = up_urls.get(base) or up_urls.get(norm) # else, we try to get the url of the uniprot protein from code_names table
            if up_url:
                g.add((prot_ind, RDFS.seeAlso, URIRef(up_url))) # we add seeAlso relation if we find a url (the real uniprot URI)
            g.add((inst_aab, BAO.BAO_0000211, prot_ind))
            g.add((prot_ind, BAO.BAO_0000598, inst_aab)) # we add relations between target instance and AAb instance
            for p in (pmids or [""]):
                add_reified_relation(g, inst_aab, BAO.BAO_0000211, prot_ind, p) # for each source we have, we add a reified relation to carry provenance
    ...
    for idx, items in data["chebi"].items():
        ...
        for chebi_raw, pmids in items: # for each chebi id associated to that aab_id
            ...
            chebi_cls = URIRef("http://purl.obolibrary.org/obo/" + code_obo) # we reconstruct the chebi class URI from the chebi id read from data
            chebi_ind = MAKAAO[code_obo + "_instance"] # we create the URI of the chebi instance
            g.add((chebi_cls, RDF.type, OWL.Class)) # we declare the chebi class as an OWL class, subclass of chebi molecule
            g.add((chebi_cls, RDFS.subClassOf, URIRef("http://purl.obolibrary.org/obo/CHEBI_23367")))
            g.add((chebi_ind, RDF.type, chebi_cls)) # we create an instance of the chebi class with the relevant URI
            ...
            chebi_url = (chebi_cn_urls or {}).get(code_colon)  # we try to get the chebi url from code_names table
            if chebi_url:
                g.add((chebi_ind, RDFS.seeAlso, URIRef(chebi_url))) # we add seeAlso relation if we find a url (the real chebi URI)
            g.add((inst_aab, BAO.BAO_0000211, chebi_ind)) # we add relations between target instance and AAb instance
            g.add((chebi_ind, BAO.BAO_0000598, inst_aab))
            for p in (pmids or [""]):
                add_reified_relation(g, inst_aab, BAO.BAO_0000211, chebi_ind, p) # for each source we have, we add a reified relation to carry provenance
                add_reified_relation(g, chebi_ind, BAO.BAO_0000598, inst_aab, p)

def process_diseases(g, data, umls_orphanet_by_cui, umls_orphanet_by_code,
                     umls_snomed_by_cui, umls_names, orpha_hpo_links, umls_cn_names):
    ...
    for idx, items in data["diseases"].items():
        ...
        for code_raw, prov in items:  # loop for every disease code associated to that aab_id in the data
            ...
            # ORPHANET
            if code_upper.startswith("ORPHA:"):  # if current disease code is an Orpha code:
                ...
                g.add((d_cls, RDFS.subClassOf, URIRef("http://www.orpha.net/ORDO/Orphanet_C001")))  # reconstruct Orphanet URI from Orpha code read in data, and add relevant triples to the graph
                g.add((d_cls, RDFS.subClassOf, MAKAAO.AutoimmuneDisease))
                for r in umls_orphanet_by_code.get(orpha_num, []):  # try to get the main english term from UMLS-Orphanet mapping to label the Orphanet code
                    s = (r.get("STR") or "").strip()
                ...
                inst = MAKAAO[f"orpha_{orpha_num}_instance"]  # also add instance of Orphanet class
                ...
                for link in orpha_hpo_links.get(str(d_cls), []):
                    ...
                    add_reified_relation(
                        g, inst, SIO["SIO_001279"], pos_inst,
                        "https://www.orphadata.com/data/xml/en_product4.xml"
                    )
                    add_reified_relation(
                        g, pos_inst, SIO["SIO_001280"], inst,
                        "https://www.orphadata.com/data/xml/en_product4.xml"
                    )
            ...
            # UMLS CUI
                ...
                inst = cui_uri
                preferred = (umls_cn_names or {}).get(code_norm)  # get preferred english label from code_names table, if it's there
                all_umls_labels = umls_names.get(code_norm) or []  # as 2nd option, get preferred eglish name from umls_names table, if it's there
                if preferred:
                    add_pref(g, cui_uri, preferred)
                elif all_umls_labels:
                    add_pref(g, cui_uri, all_umls_labels[0])
                else:
                    add_pref(g, cui_uri, code_norm)  # otherwise we add the code as a prefLabel, if we didn't get an english name
                for n in all_umls_labels:  # we add all the labels we found as regular labels
            ...
            if inst is None:
                continue
            g.add((inst, RDF.type, MAKAAO.AutoimmuneDisease))  # if no instance has been created yet, we create one.
            g.add((aab_inst, SIO["SIO_001403"], inst))         # we link that disease instance to the current aab instance
            g.add((inst, SIO["SIO_001403"], aab_inst))
            for p in (prov or [""]):
                add_reified_relation(g, aab_inst, SIO["SIO_001403"], inst, p)  # for each provenance entry, we add a reified relation to the graph, for the current instance

def process_loinc_mappings(g, loinc_umls_csv, loinc_index_csv, keep_indices):
    loinc_rows = read_csv_rows(loinc_umls_csv) # that tables contains all MRCONOS rows associated to a LOINC code
    map_rows   = read_csv_rows(loinc_index_csv) # one of our processed dat table where we mapped LOINC codes to aab_id
    if not map_rows:
        return
    loinc2cui = {}
    ...
    for r in map_rows:
        ...
        code = code.replace("LOINC:", "").replace("loinc:", "").strip() # reconstruct LOINC URI from LOINC ID read in the  data
        ...
        L = URIRef(loinc_uri)
        g.add((aab_cls, SKOS.closeMatch, L)); g.add((L, SKOS.closeMatch, aab_cls)) # link the LOINC URI to the URI of current AAb class
        cui = loinc2cui.get(code)
        if cui:
            C = URIRef(UMLS + cui)
            g.add((aab_cls, SKOS.closeMatch, C)); g.add((C, SKOS.closeMatch, aab_cls)) # also look if that LOINC code has a UMLS CUI associated to it, annd if yes, also link it to the AAb
...
def main():
    ...
    umls_names    = umls_names_index(UMLS_NAMES_CSV) if os.path.exists(UMLS_NAMES_CSV) else {}

    # read UMLS-Orphanet csv table with their main english name and syns (obtained from MRCONSO.RRF)
    orpha_by_cui, orpha_by_code = umls_code_group(UMLS_ORPHANET_CSV) if os.path.exists(UMLS_ORPHANET_CSV) else ({}, {})
    ...
    snomed_by_cui, _            = umls_code_group(UMLS_SNOMED_CSV)   if os.path.exists(UMLS_SNOMED_CSV)   else ({}, {})
    ...
    orpha_hpo_links             = grouped(read_csv_rows(ORPHANET_HPO_LINKS), "orpha_code") if os.path.exists(ORPHANET_HPO_LINKS) else {}
    ...
    if os.path.exists(CODE_NAMES_CSV):
        up_names, up_urls = read_code_names_uniprot(CODE_NAMES_CSV) # 2 dictionnaries: {uniprot_id: english_name} and {uniprot_id: uniprot_url}

        umls_cn_names, _  = read_code_names_umls(CODE_NAMES_CSV) # dictinoary: {CUI: english_name} (CUIs of differnt concept sof interest: targets, diseases...)
        chebi_cn_names, chebi_cn_urls = read_code_names_chebi(CODE_NAMES_CSV) # 2 dictionaries: {chebi_id: english_name} and {chebi_id: chebi_url}
        print(f"Loaded names — UniProt:{len(up_names)} UMLS:{len(umls_cn_names)} ChEBI:{len(chebi_cn_names)}")
    else:
        up_names, up_urls = {}, {}
        umls_cn_names = {}
        chebi_cn_names, chebi_cn_urls = {}, {}
        print(f"WARN: {CODE_NAMES_CSV} not found; labels may fall back to codes.") # if code_names file not found, just diplay a warning
    ...
    if not data["indices"]:
        ...
        raise SystemExit("No indices kept. Ensure headers include 'index' and 'name_en'.")
    ...
    build_core(g, data, umls_hpo_rows, umls_names, up_names, up_urls, umls_cn_names, chebi_cn_names, chebi_cn_urls)
    ...
    process_diseases(g, data, orpha_by_cui, orpha_by_code, snomed_by_cui, umls_names, orpha_hpo_links, umls_cn_names)
    keep = data["indices"]
    ...
    g.serialize(destination=OUTPUT_OWL_ENRICHED, format="xml") #  we write the KG to an RDF file (XML syntax)
